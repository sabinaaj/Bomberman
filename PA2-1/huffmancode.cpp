...
public:
    ...
    Tree(int value, Tree *right, Tree *left, bool valueset) {
        Value = value;
        Right = right;
        Left = left;
        ValueSet = valueset;
...
/* ------------------------------------------------------------------------------*/
    bool startMakeTree(void) {
        ...
        if (bit) return false;
        else m_root = new Tree(0, makeTree(), makeTree(), false);
        return true;
    }
    ...
    Tree *controlTree(Tree *root) {
        ...
        Tree *right = nullptr;
        if (root->Left) {
            left = controlTree(root->Left);
        }
        if (root->Right) {
            right = controlTree(root->Right);
        }
        if (!left != !right) throw 'x';
        return root;
    }
